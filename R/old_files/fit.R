...
glm.fit.hubbell <- function(x, y, weights = rep.int(1, nobs), start = NULL, etastart = NULL,
                            mustart = NULL, offset = rep.int(0, nobs), family = hubbell,
  ...
  if (EMPTY) {
    ...
    w <- sqrt((weights * mu.eta(eta, size)^2) / variance(mu, size))
    residuals <- (y - mu) / mu.eta(eta, size)
    ...
    for (iter in 1L:control$maxit) {
      ...
      eta <- drop(x %*% start)
      mu <- linkinv(eta <- eta + offset, size)
      dev <- sum(dev.resids(y, mu, weights, size))
      if (control$trace) {
  ...
  list(
    ...
    df.residual = resdf, df.null = nulldf, y = y, size = size, converged = conv,
